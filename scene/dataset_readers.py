...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, time_duration=None, fps=30):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        # Modify(Anon): read images with base path.
        # image_path = os.path.join(images_folder, os.path.basename(extr.name))
        # image_name = os.path.basename(image_path).split(".")[0]
        image_path = os.path.join(images_folder, extr.name)
        _image_path = Path(image_path).resolve()
        cam_name = _image_path.parent.stem
        img_indx = _image_path.stem
        image_name = f"{cam_name}_{img_indx}"
...
def readColmapSceneInfo(path, images, eval, llffhold=8, num_pts_ratio=1.0, time_duration=None, fps=30):
    try:
    ...
    cam_infos_unsorted = readColmapCameras(
        ...
        time_duration=time_duration, fps=fps)
